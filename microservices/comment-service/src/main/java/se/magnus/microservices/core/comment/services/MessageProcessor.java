    ...
    public void process(Event<Integer, Comment> event) {
        ...
        switch (event.getEventType()) {
            ...
            case CREATE:
                ...
                LOG.info("Create comment with ID: {}/{}", comment.getMovieId(), comment.getCommentId());
                commentService.createComment(comment);
                break;

            case DELETE:
                int movieId = event.getKey();
                LOG.info("Delete comments with movieId: {}", movieId);
                commentService.deleteComments(movieId);
                break;
